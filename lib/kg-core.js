'use strict';
/*
 * lib/kg-core.js — Le cœur de l'ATRIUM, le savoir canonique d'Hybélior.
 *
 * « L'Atrium » est le nom du savoir que ce graphe porte. Comme la salle
 * centrale de la maison antique, tout y tombe et tout y ouvre : il sait tout
 * et montre tout, c'est la source unique, celle qu'on interroge quand deux
 * textes se contredisent, et sa réponse tranche. Les livres gardent leurs
 * mystères ; l'Atrium, lui, n'en garde aucun. Quand une bible, une fiche ou
 * un chapitre le contredit, c'est l'Atrium qui a raison — et s'il a tort, on
 * le corrige ICI, pas ailleurs.
 *
...
const ENTITY_TYPES = [
  'personne', 'divinite', 'lignee', 'lieu', 'entite-politique', 'evenement', 'concept',
  'religion', 'espece', 'objet', 'terme', 'source', 'question', 'oeuvre', 'ere',
];
// Du plus grand au plus petit, puis les deux qui ne sont pas des tailles :
// une ruine est ce qui reste d'un lieu, un lieu-dit est un point sur la carte.
const LIEU_SCALES = ['monde', 'continent', 'region', 'nation', 'cite', 'ville',
  'bourg', 'hameau', 'quartier', 'ruine', 'lieu-dit'];
...
// Divulgation — DEUX axes dans un seul champ, et c'est voulu.
//   ⚠ L'ATRIUM n'est jamais restreint : il sait tout et montre tout. C'est la
//   source du savoir, celle qui permet de tout régler. `disclosure` ne gouverne
//   QUE ce qu'un narrateur de roman a le droit d'énoncer.
//   Axe « ce que le monde sait » (2026-09-22) :
//     public    — n'importe quel habitant d'Hybélior le sait.
//     restreint — un ordre, une guilde, une archive le sait ; les autres non.
//                 (Ombres Souterraines, Bureau des Concordances, archives scellées.)
//     auteur    — personne ne le sait dans le monde. Existe dans la fiction, mais
//                 AUCUN narrateur ne peut l'énoncer. Contrôlé par
//                 `scripts/verifier-divulgation.js`.
//   Axe « ce que le lecteur a le droit de voir » (hérité) :
//     t1..t4    — révélé par tel tome ; le site peut masquer au-delà.
//     interne   — valeur par défaut historique, équivalente à `auteur`, à migrer.
const DISCLOSURES = ['public', 'restreint', 'auteur', 't1', 't2', 't3', 't4', 'interne'];
...
const ID_PREFIX = {
  personne: 'per', divinite: 'div', lignee: 'lig', lieu: 'lie', 'entite-politique': 'pol',
...
function formatRange(f) {
  ...
  const fin = formatYear(f.end_year, f.end_precision, f.end_circa);
  // Une fourchette (data.fourchette) est une fenêtre d'incertitude, pas une
  // durée : « entre X et Y », jamais « X → Y ».
  if (f.data && f.data.fourchette) return `entre ${start} et ${fin}`;
  return `${start} → ${fin}`;
}
function sortKey(f) {
...
function listEntities(g, opts) {
  ...
  return es.map(e => ({ id: e.id, type: e.type, name: e.name, summary: e.summary, status: e.status, disclosure: e.disclosure, data: e.data || null }))
...
function getDossier(g, id) {
  ...
  // Le label propre d'une relation (« capitale ancienne », « doublon probable
  // — à fusionner au bake »…) qualifie le type : il s'affiche à sa suite.
  const qualif = (base, r) => r.label ? `${base} · ${r.label}` : base;
  const relOut = g.relations.filter(r => r.from_id === id).map(r => ({
    ...r, direction: 'sortante', label: qualif(r.rel_type, r), otherId: r.to_id, otherName: nameOf(r.to_id), sourceName: nameOf(r.source_id),
  }));
  const relIn = g.relations.filter(r => r.to_id === id).map(r => ({
    ...r, direction: 'entrante', label: qualif(REL_INVERSE[r.rel_type] || r.rel_type, r), otherId: r.from_id, otherName: nameOf(r.from_id), sourceName: nameOf(r.source_id),
  }));
...
// Réseau : nœuds + liens filtrés par types de relations (et optionnellement par
// types d'entités aux extrémités) — pour les visualisations croisées du portail.
function getReseau(g, opts) {
  opts = opts || {};
  const rels = opts.rels ? new Set(String(opts.rels).split(',')) : null;
  const types = opts.types ? new Set(String(opts.types).split(',')) : null;
  const maxLinks = Math.min(+opts.limit || 900, 2000);
  const links = [];
  const touchés = new Set();
  for (const r of g.relations) {
    if (rels && !rels.has(r.rel_type)) continue;
    const a = g.byId.get(r.from_id), b = g.byId.get(r.to_id);
    if (!a || !b) continue;
    if (types && !types.has(a.type) && !types.has(b.type)) continue;
    links.push({ from: r.from_id, to: r.to_id, rel: r.rel_type });
    touchés.add(r.from_id); touchés.add(r.to_id);
    if (links.length >= maxLinks) break;
  }
  const degre = {};
  for (const l of links) { degre[l.from] = (degre[l.from] || 0) + 1; degre[l.to] = (degre[l.to] || 0) + 1; }
  const nodes = [...touchés].map((id) => {
    const e = g.byId.get(id);
    return { id, name: e.name, type: e.type, degre: degre[id] || 0 };
  });
  return { nodes, links, tronque: links.length >= maxLinks };
}

// Les trajets des personnages (data.parcours), chaque étape placée sur la carte :
// coordonnées du lieu si l'auteur l'a posé (coord_x/y), sinon sa position
// estimée d'après les livres (data.carte.position_estimee), sinon celle portée
// par l'étape elle-même. L'échelle vient de l'entité d'échelle « monde ».
function positionLieu(g, id) {
  const e = id && g.byId.get(id);
  const d = e && e.data;
  if (!d) return null;
  if (d.coord_x != null && d.coord_y != null) return { x: +d.coord_x, y: +d.coord_y, estimee: false };
  const pe = d.carte && d.carte.position_estimee;
  if (pe && pe.x != null && pe.y != null) return { x: +pe.x, y: +pe.y, estimee: true };
  return null;
}
function getParcours(g) {
  const monde = g.entities.find(e => e.data && e.data.echelle === 'monde' && e.data.echelle_carte);
  const personnages = g.entities
    .filter(e => e.data && Array.isArray(e.data.parcours) && e.data.parcours.length)
    .map(e => ({
      id: e.id, name: e.name, type: e.type,
      oeuvre: e.data.parcours_oeuvre || null,
      couleur: e.data.parcours_couleur || null,
      etapes: e.data.parcours.map(p => {
        const q = positionLieu(g, p.lieu_id);
        return Object.assign({}, p, q
          ? { x: q.x, y: q.y, estimee: q.estimee }
          : { estimee: p.x != null ? p.estimee !== false : undefined });
      }),
    }));
  return { echelle: monde ? monde.data.echelle_carte : null, personnages };
}
...
function readAction(g, action, query) {
  ...
  switch (action || 'list') {
    ...
    case 'reseau': return getReseau(g, { rels: query.rels, types: query.types, limit: query.limit });
    case 'stats': return { stats: getStats(g) };
    case 'parcours': return getParcours(g);
...
module.exports = {
  ...
  getDirigeants, getArbre, getConsistencyReport, getStats, getTimelineProjection, getReseau,
  getParcours, positionLieu,
